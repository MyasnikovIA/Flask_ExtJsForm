<div caption="Примеры использования контролов"  fullscreen="true">
       ...
       <![CDATA[
           Form.getPropertyList = function(arguments) {
              ...
              refreshDataSet("DS_TREE",function(rec){
                  console.log("rec",rec)
                  console.log("rec",arr[1].data['text'] == '..')
                  let fileTree = getControl("fileTree");
                  let store = fileTree.getStore();
                  data = [];
                  for (let item = 0; item < rec.length; item++) {
                     data.push(rec[item].data);
                  }
                  store.setData(data);
                  /*
                  // дописать добавление  детей
                  if (arr[1].data['text'] === '..') {
                     store.setData(data);
                  } else {
                     lastData = store.getData();
    ...
    <cmpPanel region="west" height="40%" split="true" width="20%" minSize="100" autoScroll="true" layout="border">
        ...
        -->
        <cmpTreePanel name="fileTree"  rootVisible="false" onitemclick=" Form.getPropertyList(arguments);  " height="80%"  popupmenu="popupMenuDomTree" data="[]" >
            <colum field='text' caption='Name' />
            <colum field='value'  caption='Info' />
        </cmpTreePanel>
    </cmpPanel>
