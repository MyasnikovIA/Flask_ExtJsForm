...
def readFile(pathForm):
    ...
    if ext == "html":
        ...
        if not "lib/ExtJS_6.2.0/theme-classic-all.css" in xmlContentSrc and "</head>" in xmlContentSrc:
            fragBegin, fragEnd = xmlContentSrc.split("</head>")
            xmlContentSrc = f'{fragBegin}  <link href="lib/ExtJS_6.2.0/theme-classic-all.css" rel="stylesheet" />\r\n</head>{fragEnd}'

        # Добавляем CSS библиотеку, если её не указали на форме
        if not "lib/ExtJS_6.2.0/ext-all.js" in xmlContentSrc and "</head>" in xmlContentSrc:
            fragBegin, fragEnd = xmlContentSrc.split("</head>")
            xmlContentSrc = f'{fragBegin}  <script type="text/javascript" src="lib/ExtJS_6.2.0/ext-all.js"></script>\r\n</head>{fragEnd}'
